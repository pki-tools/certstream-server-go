...
func errorsFuncs() template.FuncMap {
...
// errorsSectionMarkup is the diagnosis panel, stats and error table without the
// page heading, shared by /errors and the combined /overview page.
const errorsSectionMarkup = `
<div class="diag">
...
// errorsSectionCSS styles this page, shared with the combined /overview page.
const errorsSectionCSS = `
...
var errorsTmpl = template.Must(template.New("errors").Funcs(errorsFuncs()).
	Parse(`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<style>` + errorsSectionCSS + `</style>
</head>
<body>` + errorsPageHeader + errorsSectionMarkup + `</body>
</html>`))

// errorsSectionTmpl renders only the section, for embedding in /overview.
var errorsSectionTmpl = template.Must(template.New("errors-section").
	Funcs(errorsFuncs()).Parse(errorsSectionMarkup))
...
// buildErrorsPageData gathers everything the page renders, split out so the
// combined /overview page can reuse it.
func buildErrorsPageData() errorsPageData {
